...
public class StatusBarUtil {
    ...
    public static void setSystemBlack(Activity activity){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //实现状态栏图标和文字颜色为暗色
            activity.getWindow().getDecorView().setSystemUiVisibility(
    ...
    public static void setSystemColor(Activity activity,String barcolor){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().setNavigationBarColor(Color.parseColor(barcolor));
            if (Build.VERSION.SDK_INT >= 28) {
                activity.getWindow().setNavigationBarDividerColor(Color.parseColor(barcolor));
            }
        }
    }
